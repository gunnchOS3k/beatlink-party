...
def run_beatlink(game: dict[str, str]) -> tuple[dict[str, Any], dict[str, dict[str, Any]]]:
    ...
    dims["loading"] = dims["launch"]
    cmds["rights_test"] = run_cmd(["pnpm", "vitest", "run", "tests/rights.test.ts"], timeout=300)
    dims["asset_validation"] = dim(
        "PASS" if cmds["rights_test"]["ok"] else "FAIL",
        "pnpm vitest run tests/rights.test.ts",
        notes="Full pnpm test suite recorded separately in COMMAND_RESULTS",
    )
    dims["audio"] = dim(
        "PARTIAL",
    ...
    dims["networking"] = dim(
        "PASS" if cmds.get("test", {}).get("ok") else "PARTIAL",
        "pnpm test (full suite) + network packages",
        None if cmds.get("test", {}).get("ok") else "HUMAN_VALIDATION_REQUIRED",
        notes="Independent verifier wait game.started failures recorded when suite flakes without redis/server",
    )
